...
~1 post per day. Visuals are preferred (chart, screenshot, meme) but text-only posts are OK for ad-hoc tasks and time-sensitive announcements.
...
### During vacation mode (April 1-14, 2026): auto-post, no CEO approval needed

1. **Fetch tone of voice** — `gh api repos/ohld/dania-zip/contents/danya-prompt.md --jq .content | base64 -d`
2. **Research** — read `experiments/reports/`, `experiments/log.jsonl`, `experiments/active/`, or use task description data
3. **Draft the post** — write in Dan's tone (see Tone of Voice below). Max 15-25 lines.
4. **Try to create visual** — use PIL/matplotlib for charts. If image generation fails, **post text-only** — do NOT skip posting.
5. **Post to Telegram** — use `curl` to call Telegram Bot API (see Posting section below). This is the MAIN deliverable. Task is NOT done until the post is sent.
6. **Verify post was sent** — check the API response for `"ok": true`
7. **Archive** — commit published post to `docs/comms/published/YYYY-MM-DD-slug.md`

**CRITICAL**: The task is NOT complete until you actually send the post via Telegram Bot API. Writing a draft is not enough.
...
**Preferred: post with visual.** If image generation fails, post text-only rather than not posting at all. Types:
...
- **Bot**: @ffmemesbot
- **15K users, 775 WAU** — small but engaged community
...
To send a text-only post (OK for ad-hoc announcements):
```bash
...
**Always check the response** — it must contain `"ok":true`. If it fails, check the error and retry.
...
- Do NOT skip posting just because visual generation failed — text-only is acceptable
