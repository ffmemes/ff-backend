...
async def conn():
    async with engine.connect() as conn:
        ...
        await conn.execute(
            ...
            [
                {'user_id': 1, 'meme_source_id': 1, 'nlikes': 10, 'ndislikes': 10, 'updated_at': datetime(2024, 1, 1, 0, 0)},
            ]
        )

        meme_stats_common = {'nlikes': 10, 'ndislikes': 10, 'age_days': 10, 'raw_impr_rank': 9999, 'sec_to_react': 10, 'invited_count': 0,
                             'updated_at': datetime(2024, 1, 1, 0, 0)}
        await conn.execute(
            insert(meme_stats),
            [
                {'meme_id': meme_id, 'lr_smoothed': -0.2 + 0.1 * meme_id, **meme_stats_common}
