...
Two layers:
  ...
  2. `write_channel_stats_report(channel, days, output_dir)` — materializes
     `experiments/reports/channel-stats-{channel}-YYYY-MM-DD.md` on demand.
...
def format_channel_stats_report(
    ...
    lines += [
        "",
        "_Generated on demand by `src/comms/performance.py`._",
        "",
    ]
    return "\n".join(lines)
...
async def write_channel_stats_report(
    ...
    """Write a channel stats markdown snapshot. Returns the file path."""
    log = get_run_logger()
