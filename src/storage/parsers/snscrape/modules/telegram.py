# CURRENT FILE VERSION: https://github.com/JustAnotherArchivist/snscrape/pull/413/files
...
import logging
import re
from src.storage.parsers.snscrape import base
import typing
import urllib.parse
...
class Channel(base.Item):
    ...
    photosGranularity = base._DeprecatedProperty('photosGranularity', lambda self: self.photos.granularity,
                                                 'photos.granularity')
    videosGranularity = base._DeprecatedProperty('videosGranularity', lambda self: self.videos.granularity,
                                                 'videos.granularity')
    linksGranularity = base._DeprecatedProperty('linksGranularity', lambda self: self.links.granularity,
                                                'links.granularity')
    filesGranularity = base._DeprecatedProperty('filesGranularity', lambda self: self.files.granularity,
                                                'files.granularity')
...
class TelegramPost(base.Item):
    ...
    views: typing.Optional[base.IntWithGranularity] = None
    ...
    def _initial_page(self):
        if self._initialPage is None:
            ...
            if r.status_code != 200:
                raise base.ScraperException(f'Got status code {r.status_code}')
    ...
    def _soup_to_items(self, soup, pageUrl, onlyUsername=False):
        ...
        for post in reversed(posts):
            ...
            yield TelegramPost(url=url, date=date, content=content, outlinks=outlinks, mentions=mentions,
                               ...
                               forwardedUrl=forwardedUrl, views=views)
    ...
    def _get_entity(self):
        ...
        for div in channelInfoDiv.find_all('div', class_='tgme_channel_info_counter'):
            ...
            if type_ == 'members':
                ...
                kwargs[type_] = base.IntWithGranularity(value, granularity)

        return Channel(**kwargs)

    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('channel', type=base.nonempty_string('channel'), help='A channel name')
