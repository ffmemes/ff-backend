...
async def handle_wrapped_button(
    ...
    if key == 2:
        ...
        if meme_data is not None:
            ...
            meme.caption = "Твои друзья больше всего орнули с этого мема 🤦‍♂️"
    ...
    if key == 4:
        await update.effective_chat.send_message(
            text="""
Спасибо большое за то, что пользуетесь ботом. Мы работаем над ним каждый день, чтобы скрасить ваши будни нашими картинками.

Продолжайте смотреть мемы и пересылать их друзьяем. С профессиональным праздником всех нас!
